...
public class Bankadmin {
	...
	public ArrayList<Customer> cancelAccount(ArrayList<Customer> array, int n) {
		if(n==0) {
			System.out.println("No more accounts to delete. For security purposes, please log in again to continue.");
		} else if(n<0 || n>array.size()) {
			System.out.println("Invalid input. For security purposes, please log in again to continue.");
		} else {
			array.remove(n-1);
			System.out.println("Account ready to be deleted...");
		}
		return array;
	}
}